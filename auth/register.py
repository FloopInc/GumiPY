...
def is_registered(user_id):A=load_user_status();return A.get(str(user_id),{}).get(_B,False)
def unregister(user_id):
	B=user_id;A=load_user_status()
	if str(B)in A:A[str(B)][_B]=False;save_user_status(A);return{_A:'Unregistration successful. User can no longer use bot commands.'}
	return{_A:'User not found.'}
